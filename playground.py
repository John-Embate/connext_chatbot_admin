...
def load_creds():
    ...
    token_file_path = os.path.join(tempfile.mkdtemp(), 'token.json')
...
def get_pdf_text(pdf_docs):
    return "".join(extract_text(pdf) for pdf in pdf_docs)
...
def app():
    ...
    if st.session_state.get("request_fine_tuned_answer"):
        fine_tuned_result = try_get_answer(st.session_state.chat_history[-1]['question'], fine_tuned_knowledge=True)
